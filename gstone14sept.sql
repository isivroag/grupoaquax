...
-- Volcando datos para la tabla gstone.material: ~6 rows (aproximadamente)
DELETE FROM `material`;
/*!40000 ALTER TABLE `material` DISABLE KEYS */;
INSERT INTO `material` (`id_mat`, `clave_mat`, `nom_mat`, `id_insumo`, `id_color`, `id_acabado`) VALUES
	(1, 'G01-AM-VENECIANO', 'VENECIANO', 1, 1, 2),
...
-- Volcando datos para la tabla gstone.precio: ~3 rows (aproximadamente)
DELETE FROM `precio`;
/*!40000 ALTER TABLE `precio` DISABLE KEYS */;
INSERT INTO `precio` (`id_precio`, `id_mat`, `unidad`, `monto`) VALUES
	(2, 2, 'ML', 3522.00),
	(3, 2, 'ml', 1000.00),
	(4, 1, 'ml', 2000.00);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vmaterial` AS SELECT material.id_mat,material.clave_mat,material.nom_mat,material.id_insumo,insumo.clave_insumo,insumo.nom_insumo,
material.id_color,color.clave_color,color.nom_color,material.id_acabado,acabado.clave_acabado,acabado.nom_acabado
FROM material
LEFT JOIN insumo ON material.id_insumo=insumo.id_insumo
LEFT JOIN color ON material.id_color=color.id_color
LEFT JOIN acabado ON material.id_acabado= acabado.id_acabado
ORDER BY material.id_mat ;
